<link rel="stylesheet" type="text/css" href="<?php echo base_url();?>boos/css/bootstrap.css">
 ...
 <?php
 foreach($infojapi->result() as $row) 
 {
        echo form_open_multipart('producto/modificarbd');
    ?>

                 <input type="hidden" name="idproducto" placeholder="Escriba el apellido paterno" class="form-control" value="<?php echo $row->idproducto;?>">
                <input type="text" name="producto" placeholder="Escriba el producto" class="form-control"  value="<?php echo $row->producto;?>">
                 <input type="text" name="gama" placeholder="Escriba la gama del producto" class="form-control"  value="<?php echo $row->gama;?>">
